...
if (isset($_POST['submitTraining'])) {
    ...
    $status = $_POST['status'];
      ...
      // Change the binding types to include 's' for strings and adjust parameters accordingly
      $insertTrainingQuery = "INSERT INTO training_programs (training_name, description, syllabus, start_date, end_date, skill_name, proficiency_level, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
      $stmt = mysqli_prepare($conn, $insertTrainingQuery);
      mysqli_stmt_bind_param($stmt, 'ssssssss', $training_name, $description, $syllabus, $start_date, $end_date, $skill, $progprof, $status);
      mysqli_stmt_execute($stmt);
        ...
        if (mysqli_stmt_affected_rows($stmt) > 0) {
            ...
            echo "<script>alert('Failed to add training program');</script>";
        }
}
